public class adbThread implements Runnable 
{
  ...
  adbThread (int waitTime, String threadName, int _server_num, String tagName, ServerSocket _serverSocket) 
  {
    ...
    correl = new Correlator(8, WINDOWSIZE, 0.8);
  }
            ...
            {   
              ...
              {      
                ...
                {              
                  ...
                  for (int d = 0; d < trialPackage.get(newSTATE).targets.size(); d++) {                      
                    targetXs.get(floor(server_num/2)).get(d).add(trialPackage.get(newSTATE).targets.get(d).getTargetPos(laptopTimestamp)[0]);
                    targetYs.get(floor(server_num/2)).get(d).add(trialPackage.get(newSTATE).targets.get(d).getTargetPos(laptopTimestamp)[1]);
                  }
...
public class Correlator
{
  ...
  public int execute( ArrayList<Float> _watchX, ArrayList<Float> _watchY, ArrayList<ArrayList<Float>> _targetsXs, ArrayList<ArrayList<Float>> _targetsYs) {
    ...
    numTargets = trialPackage.get(newSTATE).targets.size();
